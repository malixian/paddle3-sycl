...
void cinn_call_sycl_kernel(void *kernel_fn,
  ...
  
  {
    ...
    for (int idx = num_args-1; idx > 0; idx--) {
      ...
      Queue->memcpy(host_array_out, (float* )(kernel_args[idx]), 10 * sizeof(float)).wait();
      std::cout<<" ================= [CINN Debug] get sycl memory output addr: "<<kernel_args[idx]<<std::endl;
      for (size_t i = 0; i < 10; ++i) {
...
void cinn_call_cnnl_randint(void *v_args, int num_args, int seed) {
  ...
  if (dtype == cinn_int32_t()) {
    ...
    // TODO: fix range
    CNNL_CALL(cnnlRandGenerateDescreteUniform(
